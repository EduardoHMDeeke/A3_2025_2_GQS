...
public class ListaAmigosFerramentasControllerTest {
    ...
    @Test
    void testBusqueAmigoComListaVazia() throws Exception {
        Method method = ListaAmigosFerramentasController.class.getDeclaredMethod(
            "busqueAmigo", int.class, ArrayList.class);
        method.setAccessible(true);
        
        ArrayList<Amigos> listaVazia = new ArrayList<>();
        Amigos resultado = (Amigos) method.invoke(controller, 1, listaVazia);
        
        assertNull(resultado);
    }

        @Test
    void testBuscarFerramentasEncontrada() throws Exception {
        Method method = ListaAmigosFerramentasController.class.getDeclaredMethod(
            "buscarFerramentas", int.class, ArrayList.class);
        method.setAccessible(true);
        
        Ferramentas resultado = (Ferramentas) method.invoke(controller, 2, listaFerramentas);
    ...
    @Test
    void testBusqueAmigoPrimeiroElemento() throws Exception {
        Method method = ListaAmigosFerramentasController.class.getDeclaredMethod(
            "busqueAmigo", int.class, ArrayList.class);
        method.setAccessible(true);
        
        Amigos resultado = (Amigos) method.invoke(controller, 1, listaAmigos);
